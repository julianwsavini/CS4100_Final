...
# Define piano-like envelope function for decay
def envelope(t, attack=0.01, decay=0.1):
    envelope = np.exp(-t / decay) - np.exp(-t / attack)
...
silence_sound = pygame.mixer.Sound(silence_samples.tobytes())
...
def on_message(message):
    if message.type == 'note_on':
        ...
        # Create a new stereo NumPy array for the note (sine wave)
        ...
        # Apply piano-like envelope for decay
        envelope_wave = filtered_wave * envelope(t)

        # Combine left and right channels for stereo
        note_samples = np.column_stack((envelope_wave, envelope_wave))
        ...
        note_sound = pygame.mixer.Sound(note_samples.tobytes())
